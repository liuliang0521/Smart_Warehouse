...
def detect(path):
    ...
    ans = "Picture-http://172.19.252.246:8000/storehouse/pic/"+pic_name+";http://172.19.252.246:8000/storehouse/answer/"+pic_name+";"
                ...
                for index,item in enumerate(counterMap1.items()):
                    for i in range(0,len(number)):
                        if(item[0]==number[i]):
                            need[i] = item[1]
                            break
            ans = ans + ','.join(str(i) for i in need)
            print(ans)
            if True:
...
def function():
    path = request.args.get('path', 'NULL')
    print(path)
    ans = ""
    #调用函数
    with torch.no_grad():
    ...
    print(ans)
    return ans
app.run(port=7002, debug=False, host="0.0.0.0")